		...
		\section{Signalling}
			\subsection{1st Iteration - Spring Boot}
				\subsubsection*{Spring WebSockets Over STOMP}
					...
					To create the room that the users reside in, the client that uploads the file fires a request to the server using an endpoint independent of WebSockets. When the server
					receives this request, it generates a Room object and a random unique integer that serves as its identifier. This room is then added to a list of rooms and the Id is returned to the client in the response. The client then uses this id to subscribe to the room over WebSockets. When this connection is established, the server creates a user object and adds it to the correct room. This ID is then appended to the base URL of the page for the uploader to share. Once a downloader loads this url, they are redirected to a page representing the room and upon clicking either the download or stream button, the client will attempt to establish a WebSocket connection using the room id taken from the url.
				
					\begin{figure}[H]
						\caption{Spring signalling establishment sequence}
						\centering
						\includegraphics[scale=0.5]{spring-signalling-establishment-sequence.png}
					\end{figure}
			...
			\subsection{2nd Iteration - Node JS}
				...
				\subsubsection*{Dynamic Room Generation}
				\subsubsection*{Authentication}
		\section{WebRTC}
			\subsubsection{One-to-One peer connection}
			\subsubsection{Using DataChannel to transfer files}
			\subsubsection{Streaming media via peer connection}
			\subsubsection{One-to-Many peer connections}
			\subsubsection{Using GetStats library to measure performance}
		\section{Interface}
			\subsubsection{Experimenting with frameworks}
		\section{Limitations \& Improvements}
			\subsubsection{Many-to-Many peer connections}
			\subsubsection{Video Streaming}
			\subsubsection{Authentication Encryption}
			\subsubsection{Signalling server performance}
		\section{Resources used}
	\chapter{Outcomes}
		\section{Data}
		\section{Discussion}
			\subsection{Comparison to other services}
